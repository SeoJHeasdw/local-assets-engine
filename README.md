...
- **만들기**: 3D 소품·2D 이미지를 선택하고 설명을 적는다. 3D는 컨셉을 먼저 골라서 만들거나,
  ...
  보관함의 후보에서 시작할 수도 있다. 기본 생성은 고품질 원본 하나다.
- **최근 작업**: 결과 이미지와 간결한 진행 상태를 본다. 단계별 시간·메모리·로그는 작업 기록을 연다.
- **편집**: 에셋을 열어 특정 색을 찍어 교체하고 밝기·대비·채도를 조절한다. 로고 이미지를 가져오거나
  문구를 만든 뒤 에셋 표면을 클릭해 배치한다. 2D는 회전·반전·가운데 자르기도 지원한다.
  원본 비교·되돌리기가 가능하고 새 버전으로 저장한다. 미저장 편집은 자동으로 임시 보관된다.
  저장한 편집본을 다시 열면 같은 로고와 색 설정을 이어서 수정할 수 있다. 실제 형상을 깎는 조각 도구는 아니다.
